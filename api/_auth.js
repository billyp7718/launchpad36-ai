...
export function createSessionCookie(data={}){const secret=process.env.ADMIN_SECRET||'';if(!secret)throw new Error('ADMIN_SECRET is not configured');const exp=Math.floor(Date.now()/1000)+TTL;const payload=b64url(JSON.stringify({role:data.role||'member',tenant_id:data.tenant_id||null,user_id:data.user_id||null,display_name:data.display_name||'',email:data.email||'',exp}));const token=`${payload}.${sign(payload,secret)}`;return `${COOKIE}=${encodeURIComponent(token)}; Path=/; HttpOnly; Secure; SameSite=Strict; Max-Age=${TTL}`}
