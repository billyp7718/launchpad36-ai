...
export async function ensureIdentitySchema(sql=db()){
 ...
 schemaReady=(async()=>{
  ...
  await sql`create table if not exists manufacturer_team_members(
    team_id uuid not null references manufacturer_teams(id) on delete cascade,
    member_id uuid not null references manufacturer_members(id) on delete cascade,
  ...
  await sql`create table if not exists tenant_account_overlays(
    id uuid primary key default gen_random_uuid(), manufacturer_id uuid not null references manufacturers(id) on delete cascade,
...
export async function memberTeams(memberId,sql=db()){
 ...
 const rows=await sql`select t.id,t.name,tm.role from manufacturer_team_members tm join manufacturer_teams t on t.id=tm.team_id where tm.member_id=${memberId} and t.active=true order by t.name`;
 return rows;
}
