...
export async function runLivingIntelligencePipeline(input={},options={}){
  ...
  if(changed){
    event=(await sql`insert into intelligence_change_events(account_id,organization_id,subject_type,subject_key,event_type,previous_evidence_id,current_evidence_id,previous_hash,current_hash,previous_payload,current_payload,source_url,observed_at,meaningful,verification_status) values(${input.account_id||null},${input.organization_id||null},${subjectType},${subjectKey},${current?'CHANGED':'FIRST_OBSERVED'},${current?.evidence_id||null},${evidence.id},${current?.content_hash||''},${hash},${current?.payload?sql.json(current.payload):null},${sql.json(payload)},${sourceUrl},${validation.observed_at},true,${validation.verification_status}) returning *`)[0];
