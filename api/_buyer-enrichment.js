...
export async function searchApolloBuyers({domain,category=''}){
  ...
  try{const response=await fetch(url,{method:'POST',headers:{accept:'application/json','content-type':'application/json','x-api-key':key},signal:controller.signal});let body={};try{body=await response.json()}catch{}if(!response.ok)return {provider:'apollo',status:'ERROR',people:[],error:clean(body.error||body.message||`Apollo returned ${response.status}`,240),http_status:response.status,credits_used:0};const people=normalizeApolloPeople(body);return {provider:'apollo',status:people.length?'SUCCESS':'NO_RESULTS',people,total_entries:Number(body.pagination?.total_entries)||people.length,credits_used:0};}
