...
function canEditOverlay(row,tenant){return String(tenant.role||'').toLowerCase()!=='viewer'&&(canSeeAllTenantData(tenant)||String(row.owner_user_id||'')===String(tenant.user_id||'')||(row.visibility==='team'&&(tenant.team_ids||[]).includes(String(row.team_id||''))))}
...
export default async function handler(req,res){
 ...
 try{
  ...
  if(String(tenant.role||'').toLowerCase()==='viewer')return res.status(403).json({error:'Viewer access is read-only'});
