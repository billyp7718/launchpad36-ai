...
export function calculateMultiRouteMarketOpportunity({products=[],organizations=[],routes=['retail'],assumptions={},evidenceByOrganization=new Map(),buyersByOrganization=new Map()}={}){
  ...
  const effective=selected.includes('mixed')?['mixed']:selected,results=effective.map(route=>({route,result:calculateMarketOpportunity({products,organizations,route,assumptions,evidenceByOrganization,buyersByOrganization})}));
  const accounts=new Map();for(const {route,result} of results)for(const account of result.account_opportunities){const key=String(account.organization_id),existing=accounts.get(key),routesForAccount=[...new Set([...(existing?.routes_to_market||[]),route])],candidate={...account,route_to_market:route,routes_to_market:routesForAccount};if(!existing||candidate.base_manufacturer_revenue>existing.base_manufacturer_revenue)accounts.set(key,candidate);else accounts.set(key,{...existing,routes_to_market:routesForAccount})}
  const accountOpportunities=[...accounts.values()].sort((a,b)=>b.fit_score-a.fit_score||b.base_manufacturer_revenue-a.base_manufacturer_revenue),base=accountOpportunities.reduce((sum,x)=>sum+x.base_manufacturer_revenue,0),low=accountOpportunities.reduce((sum,x)=>sum+x.low_manufacturer_revenue,0),high=accountOpportunities.reduce((sum,x)=>sum+x.high_manufacturer_revenue,0),evidenceBacked=accountOpportunities.reduce((sum,x)=>sum+x.evidence_backed_manufacturer_revenue,0),retailValue=accountOpportunities.reduce((sum,x)=>sum+x.base_retail_value,0),verified=accountOpportunities.filter(x=>x.evidence_status==='VERIFIED').length,categoryTotals={};
...
async function persistWorkspaces(sql,{manufacturerId,productIds,route,result}){
  const routeScope=[...(result?.assumptions?.routes_to_market||[route])].map(String).sort().join('+'),key=`${[...productIds].map(String).sort().join('|')}::routes=${routeScope}`,saved=[];
...
export default async function handler(req,res){
  ...
  try{
    ...
    const result=calculateMultiRouteMarketOpportunity({products:hydrated,organizations,routes,assumptions:req.body?.assumptions||{},evidenceByOrganization:evidenceProfiles(evidenceRows),buyersByOrganization:buyerProfiles(buyerRows)});
