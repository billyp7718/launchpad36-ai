...
function normalizeExtractedProduct(row={}){
  const productName=clean(row.product_name,300),sku=clean(row.sku,180),explicitModel=clean(row.model_number,180);
  const modelNumber=explicitModel||sku||productName;
  return {...row,product_name:productName,sku,model_number:modelNumber,source_type:'website_discovery'};
}
...
async function extractCatalogBatch({host,urls,fetcher=fetch,timeoutMs=CATALOG_BATCH_TIMEOUT_MS}){
  ...
  const prompt=`Extract real manufacturer catalog products from these explicitly approved pages on ${host}: ${JSON.stringify(urls)}. Use current web search and only return products directly supported by those pages or their linked official product detail pages. Exclude contact, informational, support, blog, dealer, collection-only, and navigation pages unless they contain specific product records. Each row requires an exact brand and specific official product designation. A separately labeled manufacturer SKU or model number is preferred, but a clearly displayed official model designation in the product heading or product-detail URL also qualifies as model_number. For example, names such as "CXA81 Mk II", "Evo 150 SE", or other manufacturer-displayed model designations should be preserved as model_number when no separate SKU field is published. Leave sku empty when the manufacturer does not publish a separate SKU. Do not omit a real product solely because the page does not label a separate SKU or model-number field. Preserve price, UPC, image URL, features, and category only when displayed; use zero or an empty value when unknown. product_url and source_url must be official URLs actually consulted. Never invent products, identifiers, prices, features, or images.`;
  ...
  try{
    ...
    return (parsed.products||[]).map(normalizeExtractedProduct).filter(row=>row.brand&&row.product_name&&row.model_number&&normalizePublicUrl(row.source_url)).slice(0,100);
  }catch(error){
...
export async function extractCatalogPages({website,candidates,fetcher=fetch,timeoutMs=CATALOG_BATCH_TIMEOUT_MS}){
  ...
  console.info('catalog extraction completed',{host,selected_pages:urls.length,rows:rows.length,failed_pages:failed.reduce((n,x)=>n+x.urls.length,0),retried_batches:settled.filter(x=>x.attempts>1).length});
