...
export default async function handler(req,res){
  ...
  try{
    ...
    let buyers=0,products=0,evidenceAdded=0,reviewRequired=0;const errors=[];let productCollector={collector_status:'ERROR',acquisition:{}};
    if(br.ok){
      const bd=await br.json();
      for(const b of (bd.people||[])){
        if(!b.source_url)continue;
        const verificationStatus=Number(b.confidence)>=85?'VERIFIED':'REVIEW_REQUIRED';
        const living=await runLivingIntelligencePipeline({account_id:account.id,source_url:b.source_url,source_type:'corporate_page',domain:account.domain,subject_type:'buyer',subject_key:`${account.id}:${b.name}|${b.title}`,evidence_type:'buyer',payload:b,observed_at:b.verified_at||new Date().toISOString(),last_verified_at:b.verified_at||null,confidence:b.confidence||70,verification_status:verificationStatus,acquired_by:'evidence_refresh'});
        if(living.deterministic_update_applied){await upsertBuyer({account_id:account.id,name:b.name,title:b.title,email:b.email,phone:b.phone,source:b.source_label||'Public business source',source_url:b.source_url,confidence:b.confidence,verified_at:b.verified_at,status:'Current',observed_at:b.verified_at,verification_status:'VERIFIED',evidence_type:'buyer'});buyers++}else reviewRequired++;
    ...
    if(pr.ok){
      ...
      for(const p of (pd.products||[])){
        ...
        const verificationStatus=Number(p.confidence)>=85?'VERIFIED':'REVIEW_REQUIRED';
        const living=await runLivingIntelligencePipeline({account_id:account.id,source_url:p.source_url,source_type:p.source_type||'retailer_site',domain:account.domain,subject_type:'assortment_product',subject_key:`${account.id}:${p.brand}|${p.product}`,evidence_type:'assortment_product',payload:{...p,price_numeric:pn},observed_at:p.observed_at||new Date().toISOString(),last_verified_at:p.observed_at||null,confidence:p.confidence||80,verification_status:verificationStatus,acquired_by:'evidence_refresh'});
        if(living.deterministic_update_applied){await upsertCompetitiveProduct({account_id:account.id,brand:p.brand,product_name:p.product,price_text:p.price,price_numeric:pn,availability:p.availability,source_url:p.source_url,verified_at:p.observed_at||p.verified_at,raw_text:p.product,observed_at:p.observed_at,verification_status:'VERIFIED',evidence_type:'assortment_product'});products++}else reviewRequired++;
    ...
    return res.status(200).json({version:'9.8.3',account_id:accountId,account:account.name,buyers_upserted:buyers,products_upserted:products,evidence_seen:evidenceAdded,review_required:reviewRequired,product_collector:productCollector,trust:trustMetrics(evidence),pipeline:['EVIDENCE_ACQUISITION','CHANGE_DETECTION','EVIDENCE_VALIDATION','L36_INTELLIGENCE_AGENT','DETERMINISTIC_DATABASE_UPDATE'],errors});
