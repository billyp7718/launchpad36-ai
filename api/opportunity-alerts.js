...
export default async function handler(req,res){
 ...
 try{await ensureOpportunityAlerts(sql);
  if(req.method==='GET'){
   ...
   const rows=organizationId?await sql`select oa.*,ro.name organization_name from opportunity_alerts oa left join retail_organizations ro on ro.id::text=oa.organization_id::text where oa.manufacturer_id=${tenant.tenant_id} and oa.organization_id::text=${organizationId} and (${status}='' or oa.status=${status}) order by oa.impact_score desc,oa.created_at desc limit 250`:await sql`select oa.*,ro.name organization_name from opportunity_alerts oa left join retail_organizations ro on ro.id::text=oa.organization_id::text where oa.manufacturer_id=${tenant.tenant_id} and (${status}='' or oa.status=${status}) order by case oa.status when 'NEW' then 1 when 'ACKNOWLEDGED' then 2 else 3 end,oa.impact_score desc,oa.created_at desc limit 250`;
